$(function () {
	$('#hamberger, .total_menu .dim, .total_menu .close').on('click',function(e){
		...
		if (matchMedia("screen and (max-width: 1024px)").matches) { // 모바일
			...
			if ($('.total_menu').css("display") != 'block') {
				...
				//	return false; 
				//}); //sihong 20220302 - prevent 오류가 계속 발생하여 주석처리

				$(".total_nav").stop().animate({top: 0}, 300);

			}else{
				$(".total_nav").stop().animate({top: "-50%"}, 200, function(){
	...
	$('.btn_lang').on('click', function () {
		$('.btn_lang').closest('.lang_box').toggleClass('open');
	});
	$("body").on("click", function (e) {
		if (!$(e.target).hasClass("btn_lang")) {
