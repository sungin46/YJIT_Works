...
//input_POL 초기화
$(document).on("keyup", "#input_AIR_Departture", function () {
...
//퀵 Code - POL
$(document).on("click", "#input_AIR_Departture", function () {
	if ($("#input_AIR_Departture").val().length == 0) {
...
$(document).on("click", "#quick_AIR_POLCD button", function () {
	...
	$("#input_AIR_Departture").val(vSplit[0]);
...
$(document).on("keyup", "#input_AIR_Departture", function () {
	...
	$(this).autocomplete({
		...
		source: function (request, response) {
			var result = fnGetAIRPortData($("#input_AIR_Departture").val().toUpperCase());
			if (result != undefined) {
		...
		select: function (event, ui) {
			if (ui.item.value.indexOf('데이터') == -1) {
				$("#input_AIR_Departture").val(ui.item.value);
...
$(document).on("click", "#btn_AIRSchedule_Search", function () {
	...
	$('#airSchedule').css('padding-bottom', '0px');
});
...
function fnVali_AIRSchedule() {
	...
	//POL을 입력 해 주세요.
	if (_fnToNull($("input_AIR_POL").val()) == "") {
		alert("POL을 입력 해 주세요.");
		$("input_AIR_Departture").focus();
		return false;
	}

	if (_fnToNull($("input_AIR_POD").val()) == "") {
		alert("POD을 입력 해 주세요.");
